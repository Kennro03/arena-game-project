...
@export var humanoid_scene := preload("res://Scenes/Units/Humanoid/humanoid.tscn")

@export var HOTBAR_SIZE := 10
@export var autofill_test_int := 4

var inventory: Array[humanoidUnitData] = [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null]
var selected_unit_data: humanoidUnitData = null
...
func _ready() -> void:
	...
	inventory[0] = humanoidUnitData.new()
	inventory[1] = humanoidUnitData.new().get_scaled_stickmanData(0.5)
	inventory[2] = humanoidUnitData.new().get_scaled_stickmanData(1.5)
	inventory[3] = humanoidUnitData.new().get_randomized_stickmanData(0.5,1.5)
	inventory[4] = humanoidUnitData.new().get_randomized_stickmanData(1.0,3.0)
	
	
	inventory[5] = humanoidUnitData.new().get_skilled_stickmanData(1.4, skill_array_kick)
	...
	inventory[6] = humanoidUnitData.new().get_skilled_stickmanData(1.1, skill_array_kick)
...
func add_unit(unit_data: humanoidUnitData, _slot: int = -1):
...
func remove_unit(unit_data: humanoidUnitData, _slot: int = -1):
	if _slot != -1 && _slot < inventory.size():
		inventory[_slot] = null
	else : 
...
func get_hotbar_array(hotbar_length:int) -> Array[humanoidUnitData] :
	var hotbar_array : Array[humanoidUnitData]
	var i := 0
