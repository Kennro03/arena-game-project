...
func enter(_previous_state_path: String, _data := {}) -> void:
	...
	print(str(unit) + " Entered casting state. Casting " + str(cast_skill))
	
	casting_time_passed = 0.0
	if cast_skill : 
		if cast_skill.skill_data.cast_time > 0.0 :
			unit.is_casting = true
			unit.animationPlayer.play_cast_animation(1.0/cast_skill.skill_data.cast_time)
